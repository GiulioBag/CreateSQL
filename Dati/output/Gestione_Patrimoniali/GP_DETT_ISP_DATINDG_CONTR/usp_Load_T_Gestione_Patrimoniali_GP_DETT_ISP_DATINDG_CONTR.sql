...
Descrizione:
	Procedura di caricamento dalla tabella [L0].[T_Gestione_Patrimoniali_GP_DETT_ISP_DATINDG_CONTR] alla tabella [L1].[T_Gestione_Patrimoniali_GP_DETT_ISP_DATINDG_CONTR].
	Il caricamento segue una logica di MERGE (Insert + Update)
History:
	20/09/2021: Data di creazione
Esempio:
	exec [L1].[usp_Load_T_Gestione_Patrimoniali_GP_DETT_ISP_DATINDG_CONTR]
		@Exec_ID = -2147483541

============================================= 
*/

CREATE	PROCEDURE [L1].[usp_Load_T_Gestione_Patrimoniali_GP_DETT_ISP_DATINDG_CONTR]
	@Exec_Id [int]
...
;
	...
	BEGIN TRY 
		...
		UPDATE [L0].[T_Gestione_Patrimoniali_GP_DETT_ISP_DATINDG_CONTR]
...
----		FROM [L0].[T_Gestione_Patrimoniali_GP_DETT_ISP_DATINDG_CONTR] [sn]
----		JOIN ( SELECT 
...
--					,[r].[Exec_Id] ASC]
--		) as [rn]
--		FROM [L0].[T_Gestione_Patrimoniali_GP_DETT_ISP_DATINDG_CONTR] [r]
--		WHERE [Exec_Id] = @Exec_Id
--	) [sn2]
--		on
...
--
	...
	;
	INSERT INTO [L0_SCARTI].[T_Gestione_Patrimoniali_GP_DETT_ISP_DATINDG_CONTR](
		[COD_ABI]
	...
	select
		...
		,[Exec_Id]
		,[Bitmask_Scarti]
	FROM [L0].[T_Gestione_Patrimoniali_GP_DETT_ISP_DATINDG_CONTR]
	WHERE [Exec_Id] = @Exec_Id
	...
	;
	MERGE [L1].[T_Gestione_Patrimoniali_GP_DETT_ISP_DATINDG_CONTR] AS dst
	USING 
		( SELECT
			[COD_ABI]
			,[COD_NDG_CONTRATTUALE]
	...
	WHEN not matched THEN INSERT (
			...
			,[src].[COD_TIPO_NDG]
			,[src].[DES_TIPO_NDG]
			,[src].[COD_SPECIE_GIURIDICA]
			,[src].[DES_SPECIE_GIURIDICA]
			...
			,[src].[COD_SPORTELLO_CAPOFILA]
			,[src].[COD_SIGLA_PROV_RES]
			,[src].[COD_FL_STATO]
			,[src].[COD_PACCH]
			,@Exec_Id
			,@Now
	...
	WHEN matched THEN UPDATE SET
			...
			,[COD_TIPO_NDG] = [src].[COD_TIPO_NDG]
			,[DES_TIPO_NDG] = [src].[DES_TIPO_NDG]
			,[COD_SPECIE_GIURIDICA] = [src].[COD_SPECIE_GIURIDICA]
			,[DES_SPECIE_GIURIDICA] = [src].[DES_SPECIE_GIURIDICA]
			...
			,[COD_SPORTELLO_CAPOFILA] = [src].[COD_SPORTELLO_CAPOFILA]
			,[COD_SIGLA_PROV_RES] = [src].[COD_SIGLA_PROV_RES]
			,[COD_FL_STATO] = [src].[COD_FL_STATO]
			,[COD_PACCH] = [src].[COD_PACCH]
