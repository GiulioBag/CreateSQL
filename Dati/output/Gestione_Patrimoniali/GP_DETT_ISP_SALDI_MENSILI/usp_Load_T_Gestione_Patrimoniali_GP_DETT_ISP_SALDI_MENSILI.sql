...
Descrizione:
	Procedura di caricamento dalla tabella [L0].[T_Gestione_Patrimoniali_GP_DETT_ISP_SALDI_MENSILI] alla tabella [L1].[T_Gestione_Patrimoniali_GP_DETT_ISP_SALDI_MENSILI].
	Il caricamento segue una logica di MERGE (Insert + Update)
History:
	20/09/2021: Data di creazione
Esempio:
	exec [L1].[usp_Load_T_Gestione_Patrimoniali_GP_DETT_ISP_SALDI_MENSILI]
		@Exec_ID = -2147483541

============================================= 
*/

CREATE	PROCEDURE [L1].[usp_Load_T_Gestione_Patrimoniali_GP_DETT_ISP_SALDI_MENSILI]
	@Exec_Id [int]
...
;
	...
	BEGIN TRY 
		...
		UPDATE [L0].[T_Gestione_Patrimoniali_GP_DETT_ISP_SALDI_MENSILI]
...
----		FROM [L0].[T_Gestione_Patrimoniali_GP_DETT_ISP_SALDI_MENSILI] [sn]
----		JOIN ( SELECT 
...
--					,[r].[Exec_Id] ASC]
--		) as [rn]
--		FROM [L0].[T_Gestione_Patrimoniali_GP_DETT_ISP_SALDI_MENSILI] [r]
--		WHERE [Exec_Id] = @Exec_Id
--	) [sn2]
--		on
...
--
	...
	where [Exec_Id] = @Exec_Id
		...
		AND (
			...
			--([sn].[ANNO_MESE_SALDO] is null )
			--OR ([sn].[IMP_SALDO_GP] is null )
			--OR ([sn].[PRC_RENDIMENTO_ANNUALIZZATO] is null )
			--OR ([sn].[PRC_RENDIMENTO_ANNUO] is null )
		)
	; 
	...
	where [Exec_Id] = @Exec_Id
		...
		AND (
			--Inserire qui in OR tutte le date su cui si vuole effettuare la validazione
			([sn].[ANNO_MESE_SALDO] is NOT null AND TRY_CAST([ANNO_MESE_SALDO] as int) IS  NULL))
			OR ([sn].[IMP_SALDO_GP] is NOT null AND TRY_CAST(REPLACE([IMP_SALDO_GP], ',', '.') as NUMERIC (23,3)) IS  NULL))
			OR ([sn].[PRC_RENDIMENTO_ANNUALIZZATO] is NOT null AND TRY_CAST(REPLACE([PRC_RENDIMENTO_ANNUALIZZATO], ',', '.') as NUMERIC (26,9)) IS  NULL))
			OR ([sn].[PRC_RENDIMENTO_ANNUO] is NOT null AND TRY_CAST(REPLACE([PRC_RENDIMENTO_ANNUO], ',', '.') as NUMERIC (26,9)) IS  NULL))
		)
	; 
	...
	;
	INSERT INTO [L0_SCARTI].[T_Gestione_Patrimoniali_GP_DETT_ISP_SALDI_MENSILI](
		[COD_ABI]
		,[COD_CONTRATTO]
		,[COD_MANDATO]
		,[ANNO_MESE_SALDO]
		,[IMP_SALDO_GP]
		,[PRC_RENDIMENTO_ANNUALIZZATO]
		,[PRC_RENDIMENTO_ANNUO]
		,[flag_altra_pol]
		,[Row_Id]
		,[Exec_Id]
		,[Bitmask_Scarti])
	select
		...
		,[Exec_Id]
		,[Bitmask_Scarti]
	FROM [L0].[T_Gestione_Patrimoniali_GP_DETT_ISP_SALDI_MENSILI]
	WHERE [Exec_Id] = @Exec_Id
	...
	;
	MERGE [L1].[T_Gestione_Patrimoniali_GP_DETT_ISP_SALDI_MENSILI] AS dst
	USING 
		( SELECT
			[COD_ABI]
			,[COD_CONTRATTO]
			,[COD_MANDATO]
			,TRY_CAST([ANNO_MESE_SALDO] as int) 
			,TRY_CAST(REPLACE([IMP_SALDO_GP], ',', '.') as NUMERIC (23,3)) 
			,TRY_CAST(REPLACE([PRC_RENDIMENTO_ANNUALIZZATO], ',', '.') as NUMERIC (26,9)) 
			,TRY_CAST(REPLACE([PRC_RENDIMENTO_ANNUO], ',', '.') as NUMERIC (26,9)) 
			,[Row_Id]
		FROM [L0].[T_Gestione_Patrimoniali_GP_DETT_ISP_SALDI_MENSILI]
		WHERE [Exec_Id] = @Exec_Id
		AND [BitMask_Scarti] = 0
	...
	WHEN not matched THEN INSERT (
			...
			,[src].[ANNO_MESE_SALDO]
			,[src].[IMP_SALDO_GP]
			,[src].[PRC_RENDIMENTO_ANNUALIZZATO]
			,[src].[PRC_RENDIMENTO_ANNUO]
			,@Exec_Id
			,@Now
			,[src].[Row_Id])
	WHEN matched THEN UPDATE SET
			...
			,[ANNO_MESE_SALDO] = [src].[ANNO_MESE_SALDO]
			,[IMP_SALDO_GP] = [src].[IMP_SALDO_GP]
			,[PRC_RENDIMENTO_ANNUALIZZATO] = [src].[PRC_RENDIMENTO_ANNUALIZZATO]
			,[PRC_RENDIMENTO_ANNUO] = [src].[PRC_RENDIMENTO_ANNUO]
